...
public class reportController{
    ...
    /** This method generates a report on the total number of customer appointments by type and month and displays it via a lambda expression. Information is retrieved via the reporter class and displayed in a TableView, who's lambda expression populates columns. As the TableView only has one selected column with an ObservableList containing only simple strings, a method to convert strings to displayable data was required.
    ...
    /** This method generates a report of all the appointments that have been generated by a user and sorts the creation rate by date and displays it via a lambda expression. Information is retrieved via the reporter class and displayed in a TableView, who's lambda expression populates columns. As the TableView only has one selected column with an ObservableList containing only simple strings, a method to convert strings to displayable data was required.
     *
